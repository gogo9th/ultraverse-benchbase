...
public class IO2 extends Procedure {
    ...
    public void run(Connection conn, int myId, int howManyUpdatesPerTransaction, boolean makeSureWorkerSetFitsInMemory, int keyRange, WorkloadConfiguration conf) throws SQLException {
        ...
        for (int up = 0; up < howManyUpdatesPerTransaction; ++up) {
            ...
            //conf.updateQueryCount(1);
            //conf.updateTransactionCount(1);
            if (conf.statedb == 0 || key == 50)
            {  conf.executedQueryCount++;
               conf.executedTransactionCount++;
					try (PreparedStatement stmt = this.getPreparedStatement(conn, ioUpdate)) {
						 ...
						 if (result != 1) {
							  // HACK LOG.warn("supposedtochange=" + 1 + " but rc={}", result);
						 }
