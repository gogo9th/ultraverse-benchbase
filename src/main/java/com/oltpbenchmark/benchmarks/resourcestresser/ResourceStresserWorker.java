...
public class ResourceStresserWorker extends Worker<ResourceStresserBenchmark> {
    ...
    protected TransactionStatus executeWork(Connection conn, TransactionType nextTrans) throws UserAbortException, SQLException {
        if (nextTrans.getProcedureClass().equals(CPU1.class)) {
            configuration.updateQueryCount(CPU1_howManyPerTrasaction);
            ...
            configuration.updateQueryCount(CPU2_howManyPerTrasaction);
            if (configuration.statedb == 0)
            ...
            configuration.updateQueryCount(IO1_howManyRowsPerUpdate);
            if (configuration.statedb == 0 || configuration.statedb == 2)
            {   configuration.executedQueryCount += IO1_howManyUpdatePerTransaction;
                configuration.executedTransactionCount += IO1_howManyUpdatePerTransaction;
            ...
            configuration.updateQueryCount(IO2_howManyUpdatePerTransaction);
            if (configuration.statedb == 0 || configuration.statedb == 1)
            ...
            configuration.updateQueryCount(CONTENTION1_howManyKeys);
            if (configuration.statedb == 0)
            {   configuration.executedQueryCount += CONTENTION1_howManyKeys; 
            ...
            configuration.updateQueryCount(CONTENTION2_howManyUpdates);
            if (configuration.statedb == 0 || configuration.statedb == 2)
            {   
                configuration.executedQueryCount += CONTENTION2_howManyUpdates; 
    ...
    private void io1Transaction(Connection conn, int howManyColsPerRow, int howManyUpdatesPerTransaction, int howManyRowsPerUpdate, int keyRange) throws SQLException {
        ...
        proc.run(conn, this.getId(), howManyColsPerRow, howManyUpdatesPerTransaction, howManyRowsPerUpdate, keyRange/*, configuration*/);
    }
    ...
    private void io2Transaction(Connection conn, int howManyUpdatesPerTransaction, boolean makeSureWorkerSetFitsInMemory, int keyRange) throws SQLException {
        ...
        proc.run(conn, this.getId(), howManyUpdatesPerTransaction, makeSureWorkerSetFitsInMemory, keyRange, configuration);
    }
