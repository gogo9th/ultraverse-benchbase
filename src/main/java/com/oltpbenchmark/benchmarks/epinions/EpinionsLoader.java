...
public class EpinionsLoader extends Loader<EpinionsBenchmark> {
    ...
    public EpinionsLoader(EpinionsBenchmark benchmark) {
        ...
        if (this.dbrows != 0) {
            long basic_rows = EpinionsConstants.NUM_USERS + EpinionsConstants.NUM_ITEMS + EpinionsConstants.REVIEW + EpinionsConstants.TRUST;
            this.scaleFactor = this.dbrows / basic_rows;
            System.out.println("Scale Factor Changed: " + (this.scaleFactor));
        }
    ...
    private void loadItems(Connection conn, int lo, int hi) throws SQLException {
        Table catalog_tbl = this.benchmark.getCatalog().getTable("item2");
        String sql = SQLUtil.getInsertSQL(catalog_tbl, this.getDatabaseType());
