...
public class EpinionsWorker extends Worker<EpinionsBenchmark> {
    ...
    private HashMap<Integer, Integer> alias_cluster_keys_iid = null;
/*
    public final SQLStmt aliasStmt = new SQLStmt(
            "SELECT * FROM review r, item i WHERE i.i_id = r.i_id and r.i_id=? " +
                    "ORDER BY rating DESC, r.creation_date DESC LIMIT 10;"
    );
*/
/*
        if (alias_cluster_keys == null)
        {
           alias_cluster_keys = new HashMap<int, int>();
...
*/
    ...
    public void updateItemTitle(Connection conn, boolean isStateDB) throws SQLException {
        ...
        {   proc.run(conn, iid, title);
            configuration.executedQueryCount++;
            configuration.executedTransactionCount++;
        }
    }

    public void updateReviewRating(Connection conn, boolean isStateDB) throws SQLException {
