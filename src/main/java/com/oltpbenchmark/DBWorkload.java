...
public class DBWorkload {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String plugin : targetList) {
            ...
            wrkld.querycount = xmlConfig.getInt("querycount", Integer.MIN_VALUE);
            wrkld.dbrows = xmlConfig.getLong("dbrows", 0);
            wrkld.transactioncount = xmlConfig.getInt("transactioncount", Integer.MIN_VALUE);

            if (argsLine.hasOption("statedb1")) {
                System.out.println("Running StateDB...");
                wrkld.statedb = 1;
            }
            if (argsLine.hasOption("statedb2")) {
                System.out.println("Running StateDB...");
                wrkld.statedb = 2;
            }
        ...
        if (isBooleanOptionSet(argsLine, "execute")) {
            // Bombs away!
            try {
                String outputDirectory = argsLine.getOptionValue("d");
                if (outputDirectory != null && (outputDirectory.equals("/dev/null") || outputDirectory.equals("/dev/null/")))
                    ThreadBench.output = false;
                Results r = runWorkload(benchList, intervalMonitor);
                if (ThreadBench.output)
    ...
    private static Options buildOptions(XMLConfiguration pluginConfig) {
        ...
        options.addOption("statedb1", "statedb1", false, "Simulate state-db scenario 1.");
        options.addOption("statedb2", "statedb2", false, "Simulate state-db scenario 2.");
