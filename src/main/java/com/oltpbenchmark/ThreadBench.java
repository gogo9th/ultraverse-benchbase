    ...
    private Results runRateLimitedMultiPhase() {
        ...
        while (true) {
            ...
            if (phase != null) {
                ...
                // state to mark completion
                {
                    ...
                    phaseComplete = testState.getState() == State.MEASURE
                            && ((start + delta <= now) || phase.queryCountDone || phase.transactionCountDone);
                    executedQueryCount = phase.executedQueryCount;
                    executedTransactionCount = phase.executedTransactionCount;
                }
            }
            ...
            if (phaseComplete && !lastEntry) {
                ...
                resetQueues = true;
                System.out.println("Executed Query Count: " + executedQueryCount);
                System.out.println("Executed Transaction Count: " + executedTransactionCount);
                // Fetch a new Phase
                synchronized (testState) {
                    if (phase.isLatencyRun()) {
