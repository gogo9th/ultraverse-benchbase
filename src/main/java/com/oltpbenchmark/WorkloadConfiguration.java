...
public class WorkloadConfiguration {
    ...
    public int lastQueryCount = -1;
    public int lastTransactionCount = -1;
    public int executedQueryCount = 0;
    public int executedTransactionCount = 0;
    public long dbrows = 0;
    public int statedb = 0;
    public boolean output = false;
    public void updateQueryCount(int v)
    {
       Phase p = workloadState.getCurrentPhase();
       if (querycount != Integer.MIN_VALUE)
		  {   querycount -= v;
				if (querycount <= 0 && p != null)
				{  p.queryCountDone = true;
               p.executedQueryCount = executedQueryCount;
               p.executedTransactionCount = executedTransactionCount;
				}
		  }
		  if (lastQueryCount == -1 || lastQueryCount - querycount >= 10000 && p != null)
		  {   System.out.println("Query Count: " + querycount);
            lastQueryCount = querycount;
		  }
     }
    public void updateTransactionCount(int v)
    {
       Phase p = workloadState.getCurrentPhase();
       if (transactioncount != Integer.MIN_VALUE)
		  {   transactioncount -= v;
				if (transactioncount <= 0 && p != null)
				{  p.transactionCountDone = true;
               p.executedQueryCount = executedQueryCount;
               p.executedTransactionCount = executedTransactionCount;
				}
		  }
		  if (transactioncount >= 0 && lastTransactionCount == -1 || lastTransactionCount - transactioncount >= 10000 && p != null)
